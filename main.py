...
def get_configs():
    ...
    parser.add_argument("--checkpoint", type=str, default='./checkpoint/jigsaw_ped2/ped2_98.89.pth')
...
def train(args):
    ...
    # Load Data
    data_dir = f"/mnt/hdd/{args.dataset}/training/frames"
    ...
    vad_tgt_dataloader = DataLoader(vad_tgt_dataset, batch_size=32, shuffle=True, num_workers=0,
                                    pin_memory=True)

    vad_src_dataset = VideoAnomalyDataset_C3D_NM('/mnt/hdd/UBnormal_frames/training/frames',
                                                 dataset='ubnormal',
                                                 detect_dir='./detect/ubnormal_train_detect_result_yolov5.pkl',
                                                 fliter_ratio=0.5,
                                                 frame_num=args.sample_num,
                                                 static_threshold=args.static_threshold)
    ...
    vad_src_dataset = VideoAnomalyDataset_C3D_AM('/mnt/hdd/UBnormal_frames/training/frames',
                                                 dataset='ubnormal',
                                                 detect_dir='./detect/ubnormal_train_detect_result_yolov5.pkl',
                                                 fliter_ratio=0.5,
                                                 frame_num=args.sample_num,
    ...
    for epoch in range(args.epochs):
        for it, data in enumerate(vad_src_nm_dataloader):
            ...
            if iter_num % len_target == 0:
                iter_target = iter(vad_tgt_dataloader)
            data_tgt = iter_target.next()

            obj_tgt = data_tgt['obj'].cuda(args.device, non_blocking=True)
            ...
            loss_cls = F.cross_entropy(classifier(z_ori[:-len(obj_tgt)]),cls_labels) + F.cross_entropy(classifier(z_aug[:-len(obj_tgt)]),cls_labels)
            loss_grl = F.cross_entropy(classifier(z_grl), domain_label)
            # loss = loss_reg + loss_cls + loss_js*0.05
            loss = loss_cls + loss_grl
            ...
            if (global_step + 1) % args.print_interval == 0:
                print("[{}:{}/{}]\tloss: {:.6f}  Cls_loss: {:.6f}  \ttime: {:.6f}". \
                      format(epoch, it + 1, len(vad_src_nm_dataloader), loss.item(), loss_cls.item(),
                             time.time() - t0))
                # print("[{}:{}/{}]\tloss: {:.6f}  Cls_loss: {:.6f}  Reg_loss: {:.6f}  Js_loss: {:.6f}  \ttime: {:.6f}". \
                #       format(epoch, it + 1, len(vad_src_nm_dataloader), loss.item(), loss_cls.item(),
                #              loss_reg.item(), loss_js.item(),
                #              time.time() - t0))
                t0 = time.time()
            ...
            if global_step % args.val_step == 0:
                # for it, data_tgt in enumerate(vad_tgt_dataloader):
                #     obj_tgt, temp_labels, spat_labels, t_flag = data_tgt['obj'].cuda(args.device, non_blocking=True), data_tgt['label'].long().cuda(args.device), data_tgt['trans_label'].long().cuda(args.device), data_tgt['temporal']
                #
                #     z_ori, z_aug = net.fea1(obj_tgt)
                #     z_ori, z_aug = mapping.fea1(z_ori), mapping.fea1(z_aug)
                #     z_ori, z_aug = net.fea2(z_ori, z_aug)
                ...
                #     z_ori, z_aug = mapping.fea3(z_ori), mapping.fea3(z_aug)
                #     z_ori, z_aug = net.flat(z_ori), net.flat(z_aug)
                #     z_ori_out1, z_ori_out2 = tempo_classifier(z_ori), spatial_classfier(z_ori)
                #
                #     try:
                #         temp_labels = temp_labels[t_flag].view(-1)
                #         spat_labels = spat_labels[~t_flag].view(-1)
                #     except Exception:
                #         temp_labels = temp_labels[t_flag].view(-1)
                #         spat_labels = spat_labels[~t_flag].view(-1)
                #
                #     temp_loss = criterion(z_ori_out1[t_flag].view(-1,9),temp_labels)
                #     spat_loss = criterion(z_ori_out2[~t_flag].view(-1,9), spat_labels)
                #     loss_reg = mse_criterion(adaparams(z_aug - z_ori), torch.zeros_like((z_ori)))
                #
                #     loss = 1e-8 * (spat_loss+temp_loss+loss_reg)
                #
                #     optimizer_mapping.zero_grad()
                #     loss.backward()
                #     optimizer_mapping.step()
                ...
                #     writer.add_scalar('Train/spat_loss', spat_loss.item(), global_step=global_step)
                #     writer.add_scalar('Train/reg_loss', loss_reg.item(), global_step=global_step)
...
def val(args, net=None, mapping=None, classifier=None,  adaparams=None, tempo_classifier=None):
    ...
    # Load Data
    data_dir = f"/mnt/hdd/{args.dataset}/testing/frames"
