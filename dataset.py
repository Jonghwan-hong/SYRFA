...
class VideoAnomalyDataset_C3D(Dataset):
                        ...
                        if not is_small[i]:
                            ...
                            if self.dataset == 'ubnormal':
                                ...
                                if video_file.split('_')[0] == 'abnormal':
                                    track_label_path = os.path.join('/mnt/hdd/UBnormal_frames/tracks_train',
                                                                    video_file + '.txt')
    ...
    def get_object(self, video_name, frame, obj_id):
        video_dir = os.path.join('/mnt/hdd/', self.dataset, self.phase, 'patches', video_name)
        if self.dataset == 'ubnormal':
            video_dir = os.path.join('/mnt/hdd/', 'UBnormal_frames', self.phase, 'patches', video_name)
