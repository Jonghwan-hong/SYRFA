...
class GroundTruthLoader(object):
    ...
    def __load_ucsd_gt(self, dataset):
        ...
        mat_file = self.mapping[dataset]
        abnormal_events = scio.loadmat(mat_file, squeeze_me=True)['TestVideoFile']
    ...
    def __load_avenue_gt(self, dataset):
        ...
        for i in range(num_video):
            ...
            mat_path = os.path.join(root, str(video_list[i]) + '_label.mat')
            sub_video_gt = np.stack(scio.loadmat(mat_path)['volLabel'][0])
...
def compute_auc_average(res, reverse, smoothing):
    ...
    # file_name_dir = 'F:/' + dataset +  '/testing/test_frame_mask'
