...
def video_label_length(dataset='shanghaitech'):
    if dataset == 'shanghaitech':
        label_path = "/mnt/hdd/shanghaitech/testing/test_frame_mask/"
        video_length = {}
        ...
        test_frame_path = '/mnt/hdd/' + dataset + '/testing/' + 'frames/'
...
def load_objects(dataset, frame_num=7):
    root = '/mnt/hdd/'
    data_dir = os.path.join(root, dataset, 'testing','frames')
...
def remake_video_output(video_output, dataset='ped2'):
    ...
    for i in range(len(video_l)):
        ...
        # video3_ = np.ones(video_length[video])
        ...
        for fno in frame_l:
            ...
            # local_max3_ = max(object_record[:, 2].max(), local_max3_)
        ...
        # non_ones = (video3_ != 1).nonzero()[0]
        # local_max3_ = max(video3_[non_ones])
        # video3_[non_ones] = (video3_[non_ones] - min(video3_)) / (local_max3_ - min(video3_))
            
        video_ = score_smoothing(video_)
        video2_ = score_smoothing(video2_)
        # video3_ = score_smoothing(video3_)

        return_output_spatial.append(video_)
        return_output_temporal.append(video2_)
        # return_output_add.append(video3_)
